...
from .helpers import (
    ...
    condition_projection1,
    Losses,
    mask_topk_per_sample0,
    mask_topk_per_sample,
)
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, cond,epoch, if_jump,abc):
        ...
        x = condition_projection1(x, cond, self.action_dim, self.class_dim,epoch,False)
        # if epoch > 20:
        #     x[:, :, self.class_dim:self.class_dim + self.action_dim] = mask_topk_per_sample0(cond['actions'], k=5)
        ...
        if not if_jump:
            for i in reversed(range(0, self.n_timesteps)):
                ...
                x = condition_projection1(x, cond, self.action_dim, self.class_dim,epoch,True)
                #print(1)
                # if epoch > 20:
                #     x[:, :, self.class_dim:self.class_dim + self.action_dim] = mask_topk_per_sample(
                #         x[:, :, self.class_dim:self.class_dim + self.action_dim], cond['actions'], k=5)
    ...
    def p_losses(self, x_start, cond, t,epoch):
        ...
        x_noisy = condition_projection1(x_noisy, cond, self.action_dim, self.class_dim,epoch,False)
        # if epoch > 20:
        #     x_noisy[:, :, self.class_dim:self.class_dim + self.action_dim] = mask_topk_per_sample0(cond['actions'], k=5)
        x_recon = self.model(x_noisy, t)


        x_recon = condition_projection1(x_recon, cond, self.action_dim, self.class_dim,epoch,True)
        # if epoch > 20:
        #     x_noisy[:, :, self.class_dim:self.class_dim + self.action_dim] = mask_topk_per_sample(x_noisy[:, :, self.class_dim:self.class_dim + self.action_dim],cond['actions'], k=5)
        video_label=cond['video_label']


        loss = self.loss_fn(x_recon, x_start,epoch,video_label,cond) #r


        return loss

    def loss(self, x, cond,epoch):
        ...
        return self.p_losses(x, cond, t,epoch)

    def forward(self, cond,epoch,if_jump=False,abc = False):
        return self.p_sample_loop(cond,epoch, if_jump,abc)
