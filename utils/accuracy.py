...
def accuracy(output, target, topk=(1,), max_traj_len=0,dfg = False):
    with torch.no_grad():
        maxk = max(topk)
        ...
        if dfg ==True:
            correct_a1 = correct_a[:, 1].reshape(-1).float().mean().mul_(100.0)
            correct_a2 = correct_a[:, 2].reshape(-1).float().mean().mul_(100.0)
            correct_a3 = correct_a[:, 3].reshape(-1).float().mean().mul_(100.0)
            correct_a4 = correct_a[:, 4].reshape(-1).float().mean().mul_(100.0)
            return res, trajectory_success_rate, MIoU1, MIoU2, correct_a0, correct_aT,correct_a1,correct_a2,correct_a3,correct_a4
        else:
